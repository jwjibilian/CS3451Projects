...
void mouseDragged() // executed when the mouse is dragged (while mouse buttom pressed)
{
  ...
  if (keyPressed) {
    ...
    if ((key=='a')|| (key=='i') || (key=='s')) P.dragPicked();   // drag selected point with mouse
  } else {
    S.dragPicked();
  }
  change=true;
...
String title ="Spiral interpolations ", name ="Student: Joshua Jibilian and Gabriel Galarza", 
